const CustomNode = ({ node, handleClick }) => (
    <>
        <g transform={`translate(${node.x0}, ${node.y0})`} style={{ opacity: getOpacity(node) }}>
            <foreignObject width={node.width} height={50}>
                <div
                    ...
                    onMouseLeave={(e) => {
                        console.log("mouse leave", e, node);
                    }}
                >
                    {/* {node.leftIcon}  */}
                    {node.id}
...
const setBorderColor = (node) => {
    if (["a2", "b1", "c3 long long long long text"].includes(node.id)) return "1.5px solid yellow";
};

const getOpacity = (node) => {
    return node.color.split(",")[3].slice(0, -1);
};
...
const setRightIcon = (node) => {
    if (["a1", "a2", "b1", "b2", "c2"].includes(node.id))
        return (
            <g transform={`translate(140, 8) scale(0.7,0.7)`} fill="white" stroke="none">
                <path
